...
// ══════════════════════════════════════════════════════════════════════════
// FORMATO DE NÚMEROS — función unificada
// ══════════════════════════════════════════════════════════════════════════
// fmt(v, dec, opts) reemplaza las 9 funciones de formato independientes
// que cada módulo había creado. Los aliases fN, fmtNum, matFmtNum, grafFmt,
// fnF se mantienen por compatibilidad pero delegan aquí.
// Opciones: { frac: bool, sci: bool, empty: string }
// EPSILON = 1e-9 — umbral de cero compartido por todo el sistema.
// Para agregar nuevo módulo: usar fmt() directamente con opts adecuados.
// ══════════════════════════════════════════════════════════════════════════
// Reemplaza fN×2, fmtNum, matFmtNum, grafFmt, fnF
// opts: { dec=4, frac=false, sci=false, empty='—' }
// Contextos: vectores (frac mode), cálculo (sci), matrices, graficador, funciones
const EPSILON = 1e-9;
...
// Alias de compatibilidad — se eliminarán en el siguiente refactor por módulos
...
function launchModule(mod){
  ...
  if(mod==='al'){
    ...
    setTimeout(()=>{
      ...
      _emCwObserver.observe(document.getElementById('em-cw')||emCanvas.parentElement);
    },400);
  }
}
...
// • Es el graficador PRINCIPAL — el módulo fn-app (AL) fue eliminado en v1.11.0.
...
function grafUpdate() {
  ...
  if(document.getElementById('graf-canvas-wrap').classList.contains('graf-active')) {
    grafDraw();
  }
}
